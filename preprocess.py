...
def get_new_raw_files(raw_dir):
    raw_files = { "src": [], "trg": [], }
    src_files = glob.glob(os.path.join(raw_dir, "*.src"))
    raw_files["src"] = src_files
    trg_files = glob.glob(os.path.join(raw_dir, "*.trg"))
    raw_files["trg"] = trg_files
    return raw_files
...
def main_cu_bpe():
    ...
    # Download and extract raw data.
    raw_train = get_new_raw_files(opt.raw_dir)
    raw_val = get_new_raw_files(opt.raw_dir)
    raw_test = get_new_raw_files(opt.raw_dir)
    ...
    def filter_examples_with_length(x):
        print(f"src:{len(vars(x)['src'])} dst:{len(vars(x)['trg'])}")
        return len(vars(x)['src']) <= MAX_LEN and len(vars(x)['trg']) <= MAX_LEN
    ...
    print(f"{os.path.join(opt.data_dir, enc_train_files_prefix)}")
    from itertools import chain
    # field.build_vocab(chain(train.src, train.trg), min_freq=2)
